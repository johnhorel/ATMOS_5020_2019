...
# Install Python
The lab computers already have Python built in as part of the OS. 

>## Default Python on Mac OS
>Open a terminal and type `which python`.  This is the directory path to the python interpreter
>
>Now type `python` and you will enter the Python interpreter. You know you are in Python because of the `>>>` ahead of the curser. This is where you can type Python commands. 
>
>What version of Python is this? It says it is Python 2.7. This is the machines default version, but it is an old version and you can't add-on packages because you don't have admin privileges on the machine.

<img src='./images/Anaconda_Logo.png' align=right height=100> 
...
1. We want to download the 3.7 version. __Make sure you download the macOS installer_.
1. Open the installer and go through the steps.
    - When you wee the option to `change the install location`, select that you only want to install python for yourself.
1. These computers are fast. It shouldn't take more than a few minutes to install.

> You can also install Anaconda-Python on your personal computer. Refer to these instructions if you are unsure what to do.

## Anaconda Launcher
Find and open the Anaconda Launcher (pin it to the dock). This GUI will help you manage software packages we will add another day. To see what is already installed, click then `Environments` label on the side and scroll through the installed packages. If you were to install Python from the Python website, it wouldn't have installed all these other useful tools. Anaconda helps manage these packages.
...
## Install VSCode
This is a powerful text editor. There are many others, but this is a very good one that is offered through Anaconda. When it is installed, pin it to the dock. We will use this in later classes to write and edit scripts. One cool thing about VSCode is that you can open a terminal as part of the same window (more about this later).
